...
Web 应用开发的核心模块，依赖于 `SwiftBoot-Data` 模块。
...
* 接口输入参数和返回值
  ...
  ```
> 继承 `BasePopulateDto` 是为了和实体类（Entity）绑定，实现自动填充，如果不需要自动填充，直接继承 `org.swiftboot.web.request.HttpRequest` 。
...
`SwiftBoot-Data` 子模块提供了更多基于 Spring-Data-JPA 封装的数据访问支持，从而更加简便的实现数据层。
