...
以 JWT 模式为例，一旦启用了 JWT 模式，用户认证之后的用户访问令牌（Access Token）可以直接在认证接口返回给客户端（或者放到 Header 里）。
在客户端请求接口时通过`Authorization` 的请求头带上这个访问令牌，其内建的过滤器就会尝试读取这个请求头 ，并且获取 `Bearer ` 后面的 JWT 格式的访问令牌进行校验合法性，如果通过则允许访问。
...
>   revokeType: 可选 `direct` 或 `refresh`， 表示凭证撤销的方式。选 direct 的话就是直接撤销 Access Token（适用于有效期很长的情况）；
>     选 refresh 的话通过撤销 Refresh Token 的方式实现用户登录凭证的撤销（适合有效期很短的情况）
>   refreshMode: `immutable` 或 `rolling`， 表示刷新 Access Token 的时候是否同时刷新 Refresh Token，`immutable` 不刷新，`rolling` 每次都会刷新
>   storeMode: `memory` 或 `redis`，表示 JWT 存储的方式。`memory` 是默认的模式，它将 JWT 存储在内存中，仅用于开发测试；`redis` 存储 JWT 在 redis 中，需要配置 redis；
>     如果要实现自己的存储方式，例如存在数据库的用户表中，只要实现 `JwtStore` 这个接口并注册成 `@Service` 或 `@Component` 
>     参考：swiftboot-demo/swiftboot-demo-app-server/src/main/java/org/swiftboot/demo/service/JwtDatabaseStore.java

#### 实现登录接口

* Controller 

调用认证服务`UserAuthService`进行用户认证，用认证后得到的 Access Token 和超时时间创建 `AuthenticatedResponse` 对象返回即可，分配给用户的 Token 会被自动写入 Cookie 或者 Header（取决于 `useCookie` 是否为 true）
...
public AuthenticatedResponse<UserSignInDto, JwtAuthentication> appUserSign(@RequestBody NamePasswordLoginRequest request) {
    ...
    if (authenticated instanceof JwtAuthentication ja) {
        ...
        dto.setRefreshToken(ja.getRefreshToken());
        return new AuthenticatedResponse<>(dto, ja);
    }
}
```

> 自定义的 `UserSignInDto` 继承 `BaseAuthDto` 或 `BaseRefreshTokenDto`
...
```java
@Resource
private org.swiftboot.auth.filter.JwtAuthFilter jwtAuthFilter;
...
实现退出登录接口需要返回 `LogoutResponse` ，这样 SwiftBoot-Auth 就会自动撤销退出登录后的 JWT。
注意：这个接口不需要配置成让 `JwtAuthFilter` 过滤，因为如果 access token 过期的话，撤销逻辑就永远不会执行。
