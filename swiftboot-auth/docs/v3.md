...
public Response<UserSignInDto> appUserSign(@RequestBody NamePasswordLoginRequest request) {
    ...
    if (authenticated instanceof JwtAuthentication ja) {
        ...
        dto.setAccessToken(ja.getAccessToken().getTokenValue());
        dto.setExpiresAt(ja.getAccessToken().getExpiresAt());
        return new AuthenticatedResponse<>(dto, ja);
    }
}
```

> `UserSignInDto` 继承 `BaseAuthDto` 或 `BaseRefreshTokenDto`
