...
@EnableJpaRepositories(basePackages = {"com.example.myproject"})
...
和 Spring Data JPA 所要求的一样，所有 Repository 接口继承 `org.springframework.data.repository.Repository` 接口或者其子接口，例如：
...
public class OrderEntity extends BaseIdEntity {
  ...
  ```
> 所有继承 `BaseIdEntity` 的实体类都有一个名为 `id` 的属性作为主键，如果还想想给实体类加上创建时间和修改时间，可以继承`BaseLocalDateTimeEntity`或者`BaseLongTimeEntity`。
> 如果不希望使用继承而产生耦合，但又希望利用到 SwiftBoot 对 ID 主键的处理，也可以让你的实体类实现 `IdPersistable` 接口。
> 默认情况下主键字段 `id` 不定义长度（采用数据库默认的长度），如果需要统一指定它的长度，参考高级主题： [主键长度](v3_advanced.md#主键长度)
